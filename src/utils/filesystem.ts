...
export const writeFile = async ({
  ...
  const replace = (str: string): string =>
    [
      ...
      ...(section === 'extension' ? [config.extension.replacer as unknown as Replacer] : []),
      ...(section?.includes('migration') ? [config.migrations.replacer as unknown as Replacer] : []),
...
export const readFile = async (location: string): Promise<string> => {
